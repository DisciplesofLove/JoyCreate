...
/**
 * A webpack loader that adds `data-joy-*` attributes to JSX elements.
 */
export default function joyTaggerLoader(this: any, code: string) {
          ...
          try {
            ...
            const alreadyTagged = node.attributes?.some(
              (attr: any) =>
                ...
                (attr.name?.name === "data-joy-id" || attr.name?.name === "data-dyad-id"),
            );
            ...
            if (!loc) return;
            const joyId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-joy-id="${joyId}" data-joy-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
  ...
  transform()
    ...
    .catch((err) => {
      console.error(`[joy-tagger] ERROR in ${this.resourcePath}:`, err);
