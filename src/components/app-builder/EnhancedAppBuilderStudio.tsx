...
import { lastBuildBriefAtom } from "@/atoms/quickStartAtoms";
...
export function EnhancedAppBuilderStudio() {
  ...
  // Seed from the most recent Quick Start brief once, so a build that started
  // in the Quick Start tab opens here pre-configured. Non-destructive: only
  // overrides the default "chat" mode.
  const lastBrief = useAtomValue(lastBuildBriefAtom);
  const seededRef = useRef(false);
  useEffect(() => {
    if (seededRef.current || !lastBrief) return;
    seededRef.current = true;
    const mode = lastBrief.buildMode as BuildMode | undefined;
    if (mode && mode !== buildMode) {
      const valid: BuildMode[] = [
        "chat",
        "agent",
        "plan",
        "visual",
      ...
      if (valid.includes(mode)) setBuildMode(mode);
    }
