/**
 * AccountBillingPanel — body of the former /profile page (UserProfilePage)
 * minus the public-facing parts (which now live in PublicProfilePanel).
 *
 * Sub-tabs: Profile basics | Account | Usage & Billing | Connected services
 *
      ...
      <div className="flex items-start gap-6">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div>
            ...
            <Textarea
              defaultValue="Builder and entrepreneur. Creating the future with AI."
              className="mt-1"
              rows={3}
            />
          </div>
    ...
    <div className="space-y-6 max-w-2xl">
      <Card className="bg-muted/10 border-border/30">
        <CardHeader className="py-3 px-4">
          <CardTitle className="text-sm">Email & Password</CardTitle>
        </CardHeader>
      ...
      <Card className="bg-muted/10 border-border/30">
        ...
        <CardContent className="px-4 pb-4">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm">
              <Shield className="w-3.5 h-3.5 mr-1" /> Enable 2FA
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-muted/10 border-border/30">
        <CardHeader className="py-3 px-4">
          <CardTitle className="text-sm">Connected Wallets</CardTitle>
        </CardHeader>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Storage Used", value: "2.1 GB", limit: "50 GB", pct: 4 },
          {
...
function ConnectedServicesTab() {
  const services = [
    ...
    },
    {
      ...
      connected: true,
      account: "hello@joymarketplace.io",
    },
    {
      name: "OpenClaw",
      icon: <Cpu className="w-5 h-5" />,
      connected: true,
      account: "Local Gateway",
    },
    {
      name: "Ollama",
      icon: <Cpu className="w-5 h-5" />,
      connected: true,
      account: "12+ models loaded",
    },
  ];
          ...
          <CardContent className="p-3 flex items-center gap-3">
            ...
            {svc.connected ? (
              ...
              <Button variant="outline" size="sm" className="h-7 text-[10px]">
                <LinkIcon className="w-3 h-3 mr-0.5" /> Connect
              </Button>
            )}
          </CardContent>
        </Card>
    ...
    >
      <TabsList className="bg-transparent">
        <TabsTrigger value="profile" className="gap-1.5">
          <User className="w-3.5 h-3.5" /> Profile basics
        </TabsTrigger>
        <TabsTrigger value="account" className="gap-1.5">
          <Key className="w-3.5 h-3.5" /> Account
        </TabsTrigger>
        <TabsTrigger value="usage" className="gap-1.5">
          <BarChart3 className="w-3.5 h-3.5" /> Usage & Billing
        </TabsTrigger>
        <TabsTrigger value="connected" className="gap-1.5">
          <LinkIcon className="w-3.5 h-3.5" /> Connected
        </TabsTrigger>
      </TabsList>
      <div className="pt-4">
