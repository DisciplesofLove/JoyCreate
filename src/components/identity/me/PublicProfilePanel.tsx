/**
 * PublicProfilePanel — body of the former /creator-profile page,
 * reused inside /identity?tab=public.
 *
...
export function PublicProfilePanel() {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl bg-gradient-to-br from-violet-500/20 via-purple-500/10 to-pink-500/20 border p-6">
        <div className="flex items-start gap-6">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-violet-500 to-pink-500 flex items-center justify-center text-white text-2xl font-bold shadow-lg">
            T
          </div>
          <div className="flex-1">
            <h2 className="text-xl font-bold">Terry</h2>
            <p className="text-sm text-muted-foreground mt-1">
              Builder and entrepreneur. Creating the future with AI and
              sovereign technology.
            </p>
            <div className="flex flex-wrap items-center gap-4 mt-3">
              <span className="text-[11px] text-muted-foreground/60 flex items-center gap-1">
                <Calendar className="w-3 h-3" /> Joined Apr 2026
              </span>
              <span className="text-[11px] text-muted-foreground/60 flex items-center gap-1">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3">
              {[
                ...
                <Badge
                  key={badge.label}
                  className={`${badge.color} text-[9px]`}
                >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-transparent">
          ...
          <TabsTrigger value="apps" className="gap-1.5">
            <Rocket className="w-3.5 h-3.5" /> Apps
          </TabsTrigger>
          <TabsTrigger value="templates" className="gap-1.5">
            <Package className="w-3.5 h-3.5" /> Templates
          </TabsTrigger>
        </TabsList>
          ...
          {agents.map((agent) => (
            <Card key={agent.name} className="bg-muted/10 border-border/30">
              <CardContent className="p-3 flex items-center gap-3">
                ...
                >
                  <Download className="w-3 h-3 mr-0.5" /> Install
                </Button>
              </CardContent>
            </Card>
          ))}
