...
export const MCP_SERVER_REGISTRY: McpServerRegistryEntry[] = [
  // ===== FEATURED SERVERS =====
  {
    ...
    name: "n8n (local)",
    description:
      "Connect to your local n8n instance to run, trigger, and orchestrate workflows as MCP tools.",
    longDescription:
      "n8n is a workflow automation platform with hundreds of integrations. Installing this from MCP Hub will automatically create + activate an MCP Server Trigger workflow named \"JoyCreate MCP Server\" inside your local n8n, then connect to it. Open n8n at http://localhost:5678 and wire tool nodes (HTTP Request, Gmail, Slack, Postgres, etc.) downstream of that trigger to expose them as callable tools in JoyCreate.",
    category: "featured",
    featured: true,
    config: {
      type: "http",
    ...
    tags: ["workflow", "automation", "integrations", "n8n"],
    author: "n8n",
    requiresAuth: true,
    envVars: [
      {
        key: "X-N8N-API-KEY",
        description:
          "n8n public API key (Settings → API) — used as an HTTP header on MCP requests.",
        required: true,
  ...
  // ===== OFFICIAL MCP REFERENCE SERVERS (@modelcontextprotocol/server-*) =====
  {
    id: "memory",
    name: "Memory",
    description: "Persistent knowledge-graph memory the assistant can read and write.",
    longDescription: "Official MCP reference server providing a persistent knowledge graph the assistant can use to remember facts, entities, and relations across sessions.",
    category: "productivity",
    featured: true,
    config: {
      type: "stdio",
  ...
  },
  {
    id: "fetch",
    name: "Fetch",
    description: "Fetch URLs and convert HTML to clean Markdown for the assistant.",
  ...
  },
  {
    ...
    description: "Structured step-by-step reasoning scratchpad for the assistant.",
    longDescription: "Official MCP sequential-thinking server. Provides a tool for the model to think through hard problems one step at a time, with branch and revise.",
    category: "ai-assistants",
    featured: true,
    config: {
      type: "stdio",
    ...
    tags: ["reasoning", "planning", "reference"],
    author: "Model Context Protocol",
  },
  {
    id: "git",
  ...
  },
  {
    ...
    description: "Reference server exercising every MCP feature — useful for testing and demos.",
    longDescription: "Official MCP 'everything' demo server: exposes sample tools, resources, prompts, and notifications so you can verify your client supports the full spec.",
    category: "development",
    config: {
      type: "stdio",
    ...
    tags: ["reference", "demo", "testing"],
    author: "Model Context Protocol",
  },
  {
    id: "postgres",
    name: "Postgres",
    description: "Read-only access to any Postgres database via connection string.",
    longDescription: "Official MCP postgres server. Connects to any Postgres instance with a standard DATABASE_URL and exposes schema introspection plus read-only SQL query tools.",
    category: "databases",
    config: {
      type: "stdio",
    ...
    tags: ["postgres", "sql", "database", "reference"],
    author: "Model Context Protocol",
    requiresAuth: true,
    envVars: [
      {
  ...
  },
  {
    id: "redis",
    name: "Redis",
    description: "Read and write keys against any Redis-compatible server.",
    longDescription: "Official MCP redis server. Standard Redis tools (get, set, del, scan, hash, list, etc.) over a configured REDIS_URL.",
    category: "databases",
    config: {
      type: "stdio",
  ...
  },
  {
    ...
    longDescription: "Official MCP gdrive server. List, search, and read documents from Drive (Docs, Sheets, Slides, PDFs). Requires OAuth credentials.",
    category: "productivity",
    config: {
      type: "stdio",
    ...
    tags: ["gdrive", "documents", "reference"],
    author: "Model Context Protocol",
    requiresAuth: true,
    envVars: [
      {
  ...
  },
  {
    ...
    longDescription: "Official MCP brave-search server. Performs general web search and locality-aware queries; great for research workflows.",
    category: "data-processing",
    config: {
      type: "stdio",
  ...
  // ===== POPULAR THIRD-PARTY SEARCH / RESEARCH =====
  {
    id: "tavily",
    name: "Tavily Search",
    description: "AI-optimised web search with extracted, summarised results.",
    longDescription: "Tavily provides search APIs tailored for LLM workflows — it returns clean, summarised snippets and supports deep research mode.",
    category: "data-processing",
    config: {
      type: "stdio",
  ...
  },
  {
    id: "exa",
    name: "Exa Search",
    description: "Neural search engine for finding high-quality web pages.",
    longDescription: "Exa MCP server exposes Exa's neural search API, useful for finding authoritative web content beyond keyword search.",
    category: "data-processing",
    config: {
      type: "stdio",
  ...
  },
  {
    id: "firecrawl",
    name: "Firecrawl",
    description: "Crawl and scrape websites into clean Markdown for LLMs.",
    longDescription: "Firecrawl MCP server crawls entire sites or single URLs and returns LLM-ready Markdown plus structured extraction.",
    category: "data-processing",
    config: {
      type: "stdio",
  ...
  },
  {
    id: "perplexity-ask",
    name: "Perplexity Ask",
    description: "Ask Perplexity's online models for cited, up-to-date answers.",
    longDescription: "Perplexity MCP server lets the assistant query Perplexity's online models (sonar) and receive answers with web citations.",
    category: "ai-assistants",
    config: {
      type: "stdio",
