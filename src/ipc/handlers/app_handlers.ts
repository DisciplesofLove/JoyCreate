...
function listenToProcess({
  ...
  spawnedProcess.stdout?.on("data", async (data) => {
    ...
    // isNeon because: 1) only Neon apps (for the official JoyCreate templates) should
...
async function executeAppInDocker({
  ...
  const containerName = `joy-app-${appId}`;
  ...
  const dockerfilePath = path.join(appPath, "Dockerfile.joy");
...
`;
  ...
  const buildProcess = spawn(
    "docker",
    ["build", "-f", "Dockerfile.joy", "-t", `joy-app-${appId}`, "."],
    {
      cwd: appPath,
      stdio: "pipe",
    },
...
export function registerAppHandlers() {
  ...
  handle(
    ...
    async (
      ...
      const fullAppPath = getJoyAppPath(appPath);
  ...
  handle(
    ...
    async (_, params: CopyAppParams): Promise<{ app: any }> => {
      ...
      const originalAppPath = getJoyAppPath(originalApp.path);
      const newAppPath = getJoyAppPath(newAppName);
  ...
  ipcMain.handle("list-apps", async () => {
    ...
    return {
      apps: allApps,
      appBasePath: getJoyAppPath("$APP_BASE_PATH"),
    };
  });
      ...
      return withLock(appId, async () => {
        try {
          ...
          if (removeNodeModules) {
            ...
            if (runtimeMode === "docker") {
              logger.log(
                `Docker mode detected for app ${appId}. Removing Docker volumes joy-pnpm-${appId}...`,
              );
              try {
                await removeDockerVolumesForApp(appId);
                logger.log(
                  `Removed Docker volumes for app ${appId} (joy-pnpm-${appId}).`,
                );
              } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return withLock(appId, async () => {
        ...
        const oldAppPath = getJoyAppPath(app.path);
        const newAppPath = getJoyAppPath(appPath);
  ...
  ipcMain.handle("reset-all", async (): Promise<void> => {
    ...
    const joyAppPath = getJoyAppPath(".");
    if (fs.existsSync(joyAppPath)) {
      await fsPromises.rm(joyAppPath, { recursive: true, force: true });
      // Recreate the base directory
      await fsPromises.mkdir(joyAppPath, { recursive: true });
    }
