...
const TEMP_DIR = path.join(os.tmpdir(), "joy-attachments");
...
async function processStreamChunks({
  ...
  for await (const part of fullStream) {
    ...
    if (part.type === "text-delta") {
      ...
      chunk += escapeJoyTags(part.text);
    } else if (part.type === "tool-call") {
      const { serverName, toolName } = parseMcpToolKey(part.toolName);
      const content = escapeJoyTags(JSON.stringify(part.input));
      chunk = `<joy-mcp-tool-call server="${serverName}" tool="${toolName}">\n${content}\n</joy-mcp-tool-call>\n`;
    } else if (part.type === "tool-result") {
      const { serverName, toolName } = parseMcpToolKey(part.toolName);
      const content = escapeJoyTags(part.output);
      chunk = `<joy-mcp-tool-result server="${serverName}" tool="${toolName}">\n${content}\n</joy-mcp-tool-result>\n`;
    }
...
export function registerChatStreamHandlers() {
  ipcMain.handle("chat:stream", async (event, req: ChatStreamParams) => {
    ...
    try {
      ...
      let joyRequestId: string | undefined;
      ...
      if (req.attachments && req.attachments.length > 0) {
        ...
        for (const [index, attachment] of req.attachments.entries()) {
          ...
          if (attachment.attachmentType === "upload-to-codebase") {
            ...
            const fileId = `JOY_ATTACHMENT_${index}`;
            ...
            // Add instruction for AI to use joy-write tag
            ...
            if (await isTextFile(filePath)) {
              try {
                attachmentInfo += `<joy-text-attachment filename="${attachment.name}" type="${attachment.type}" path="${filePath}">
                </joy-text-attachment>
                \n\n`;
              } catch (err) {
        ...
        for (const component of componentsToProcess) {
          ...
          try {
            const componentFileContent = await readFile(
              path.join(getJoyAppPath(chat.app.path), component.relativePath),
              "utf8",
            );
...
`;
      ...
      const [placeholderAssistantMessage] = await db
        ...
        .values({
          ...
          requestId: joyRequestId,
          model: settings.selectedModel.name,
          sourceCommitHash: await getCurrentCommitHash({
            path: getJoyAppPath(chat.app.path),
          }),
        })
        .returning();
      ...
      if (testResponse) {
        ...
        // additional files through <joy-read> as needed.
        if (
          isSmartContextEnabled &&
          req.selectedComponents &&
          req.selectedComponents.length > 0
        ...
        // For JoyCreate Pro + Deep Context, we set to 200 chat turns (+1)
        ...
        const aiRules = await readAiRules(getJoyAppPath(updatedChat.app.path));
...
<joy-write path="path/to/destination/filename.ext" description="Upload file to codebase">
JOY_ATTACHMENT_X
</joy-write>

Example for file with id of JOY_ATTACHMENT_0:
<joy-write path="src/components/Button.jsx" description="Upload file to codebase">
JOY_ATTACHMENT_0
</joy-write>
...
`;
        ...
        const limitedHistoryChatMessages = limitedMessageHistory.map((msg) => ({
          ...
          content:
            settings.selectedChatMode === "ask"
              ? removeJoyTags(removeNonEssentialTags(msg.content))
        ...
        if (settings.selectedChatMode === "agent" && !isLocal) {
          ...
          const { fullStream } = await simpleStreamText({
            ...
            systemPromptOverride: constructSystemPrompt({
              aiRules: await readAiRules(getJoyAppPath(updatedChat.app.path)),
              chatMode: "agent",
        ...
        try {
          ...
          if (
            ...
            while (
              ...
              fullResponse += `<joy-output type="warning" message="Could not apply Turbo Edits properly for some of the files; re-generating code...">${formattedSearchReplaceIssues}</joy-output>`;
              ...
              const fixSearchReplacePrompt =
                searchReplaceFixAttempts === 0
                  ? `There was an issue with the following \`joy-search-replace\` tags. Make sure you use \`joy-read\` to read the latest version of the file and then trying to do search & replace again.`
                  : `There was an issue with the following \`joy-search-replace\` tags. Please fix the errors by generating the code changes using \`joy-write\` tags instead.`;
...
${formattedSearchReplaceIssues}`,
          ...
          if (
            ...
            while (
              ...
              logger.warn(
                `Received unclosed joy-write tag, attempting to continue, attempt #${continuationAttempts + 1}`,
              );
          ...
          const addDependencies = getJoyAddDependencyTags(fullResponse);
          if (
            !abortController.signal.aborted &&
...
${problemReport.problems
  ...
  .join("\n")}
                ...
                const virtualFileSystem = new AsyncVirtualFileSystem(
                  getJoyAppPath(updatedChat.app.path),
                  {
...
[Response cancelled by user]`,
      ...
      if (!abortController.signal.aborted && fullResponse) {
        // Scrape from: <joy-chat-summary>Renaming profile file</joy-chat-title>
        const chatTitle = fullResponse.match(
          /<joy-chat-summary>(.*?)<\/joy-chat-summary>/,
        );
        if (chatTitle) {
          await db
            .update(chats)
  ...
  try {
    if (await isTextFile(filePath)) {
      ...
      const tagPattern = new RegExp(
        `<joy-text-attachment filename="[^"]*" type="[^"]*" path="${escapedPath}">\\s*<\\/joy-text-attachment>`,
        "g",
      );
...
export function removeProblemReportTags(text: string): string {
  const problemReportRegex =
    /<joy-problem-report[^>]*>[\s\S]*?<\/joy-problem-report>/g;
  return text.replace(problemReportRegex, "").trim();
}

export function removeJoyTags(text: string): string {
  const joyRegex = /<joy-[^>]*>[\s\S]*?<\/joy-[^>]*>/g;
  return text.replace(joyRegex, "").trim();
}

export function hasUnclosedJoyWrite(text: string): boolean {
  // Find the last opening joy-write tag
  const openRegex = /<joy-write[^>]*>/g;
  let lastOpenIndex = -1;
  let match;
  ...
  const hasClosingTag = /<\/joy-write>/.test(textAfterLastOpen);

  return !hasClosingTag;
}

function escapeJoyTags(text: string): string {
  // Escape joy tags in reasoning content
  // We are replacing the opening tag with a look-alike character
  // to avoid issues where thinking content includes joy tags
  // and are mishandled by:
  // 1. FE markdown parser
  // 2. Main process response processor
  return text.replace(/<joy/g, "＜joy").replace(/<\/joy/g, "＜/joy");
}
