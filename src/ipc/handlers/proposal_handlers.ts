  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getJoyChatSummaryTag(messageContent);

        const proposalWriteFiles = getJoyWriteTags(messageContent);
        const proposalSearchReplaceFiles =
          getJoySearchReplaceTags(messageContent);
        const proposalRenameFiles = getJoyRenameTags(messageContent);
        const proposalDeleteFiles = getJoyDeleteTags(messageContent);
        const proposalExecuteSqlQueries = getJoyExecuteSqlTags(messageContent);
        const packagesAdded = getJoyAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getJoyWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getJoyCommandTags(latestAssistantMessage.content);
...
const approveProposalHandler = async (
  ...
  const chatSummary = getJoyChatSummaryTag(messageToApprove.content);
