...
import {
  ...
  getJoyExecuteSqlTags,
  getJoySearchReplaceTags,
...
export async function dryRunSearchReplace({
  ...
  const joySearchReplaceTags = getJoySearchReplaceTags(fullResponse);
  for (const tag of joySearchReplaceTags) {
...
export async function processFullResponseActions(
  ...
  const appPath = getJoyAppPath(chatWithApp.app.path);
  ...
  try {
    // Extract all tags
    const joyWriteTags = getJoyWriteTags(fullResponse);
    const joyRenameTags = getJoyRenameTags(fullResponse);
    const joyDeletePaths = getJoyDeleteTags(fullResponse);
    const joyAddDependencyPackages = getJoyAddDependencyTags(fullResponse);
    const joyExecuteSqlQueries = chatWithApp.app.supabaseProjectId
      ? getJoyExecuteSqlTags(fullResponse)
      : [];
    ...
    if (joyExecuteSqlQueries.length > 0) {
      for (const query of joyExecuteSqlQueries) {
        try {
      ...
      logger.log(`Executed ${joyExecuteSqlQueries.length} SQL queries`);
    }

    // TODO: Handle add dependency tags
    if (joyAddDependencyPackages.length > 0) {
      try {
        await executeAddDependency({
          packages: joyAddDependencyPackages,
          message: message,
          appPath,
        });
      } catch (error) {
        errors.push({
          message: `Failed to add dependencies: ${joyAddDependencyPackages.join(
            ", ",
          )}`,
          error: error,
        });
    ...
    for (const filePath of joyDeletePaths) {
      const fullFilePath = safeJoin(appPath, filePath);
    ...
    for (const tag of joyRenameTags) {
    ...
    for (const tag of joySearchReplaceTags) {
      ...
      try {
        if (!fs.existsSync(fullFilePath)) {
          // Do not show warning to user because we already attempt to do a <joy-write> tag to fix it.
        ...
        const result = applySearchReplace(original, tag.content);
        if (!result.success || typeof result.content !== "string") {
          // Do not show warning to user because we already attempt to do a <joy-write> and/or a subsequent <joy-search-replace> tag to fix it.
          logger.warn(
    ...
    if (hasChanges) {
      ...
      if (joyAddDependencyPackages.length > 0)
        changes.push(
          `added ${joyAddDependencyPackages.join(", ")} package(s)`,
        );
      if (joyExecuteSqlQueries.length > 0)
        changes.push(`executed ${joyExecuteSqlQueries.length} SQL queries`);

      let message = chatSummary
        ? `[joy] ${chatSummary} - ${changes.join(", ")}`
        : `[joy] ${changes.join(", ")}`;
      ...
      if (uncommittedFiles.length > 0) {
        ...
        try {
          commitHash = await gitCommit({
            path: appPath,
            message: message + " + extra files edited outside of JoyCreate",
            amend: true,
          });
          logger.log(
            `Amend commit with changes outside of JoyCreate: ${uncommittedFiles.join(", ")}`,
          );
        } catch (error) {
          // Just log, but don't throw an error because the user can still
          // commit these changes outside of JoyCreate if needed.
          logger.error(
            `Failed to commit changes outside of JoyCreate: ${uncommittedFiles.join(
              ", ",
            )}`,
          );
    ...
    ${warnings
      .map(
        (warning) =>
          `<joy-output type="warning" message="${warning.message}">${warning.error}</joy-output>`,
      )
      .join("\n")}
    ${errors
      .map(
        (error) =>
          `<joy-output type="error" message="${error.message}">${error.error}</joy-output>`,
      )
      .join("\n")}
    `;
