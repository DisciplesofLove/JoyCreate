...
const SUPPORTED_PROVIDERS = ["supabase", "neon"] as const;

const addIntegrationSchema = z.object({
  provider: z
    .enum(SUPPORTED_PROVIDERS)
    .describe(
      "The integration provider to add: 'supabase' (auth, database, edge functions) or 'neon' (serverless Postgres database)",
    ),
});
...
> = {
  ...
  description:
    "Add an integration provider to the app: 'supabase' (auth, database, or server-side functions) or 'neon' (serverless Postgres database). Once you have called this tool, stop and do not call any more tools because you need to wait for the user to set up the integration.",
  ...
  buildXml: (args, _isComplete) => {
    ...
    return `<joy-add-integration provider="${escapeXmlAttr(args.provider)}"></joy-add-integration>`;
  },
